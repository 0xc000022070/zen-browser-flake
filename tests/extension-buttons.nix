{
  ...
  testScript =
    wrapWithX11
    ''
      # Zen saves the layout only after the first window paints, which on a
      # slow CI runner can outlive a single fixed timeout.
      def launch_zen_until(check):
          for t in (60, 90, 120):
              machine.succeed(
                  f"su - testuser -c 'DISPLAY=:99 timeout {t} zen-twilight about:blank' || true"
              )
              machine.succeed("sleep 2")
              if machine.execute(check)[0] == 0:
                  return
          raise Exception(f"zen-twilight launches never satisfied: {check}")
      ...
      # Launch Zen until it saves a layout to merge into.
      launch_zen_until(
      ...
      # The placements must survive a real browser round-trip even though none
      # of these extensions are installed; a diff against /tmp/prefs.before
      # proves Zen rewrote the file.
      launch_zen_until(
        "! cmp -s /tmp/prefs.before /home/testuser/.config/zen/default/prefs.js"
        " && grep -q '^user_pref(\"browser.uiCustomization.state\", '"
        " /home/testuser/.config/zen/default/prefs.js"
      )
